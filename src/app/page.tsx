...
export default function Home() {
  ...
  async function handleEmailAuth() {
    ...
    if (authMode === "forgot") {
      if (!email) { setNotice("Enter your email address."); return; }
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: window.location.hostname === "localhost" ? window.location.origin : productionAuthRedirect,
      });
    ...
    if (authMode === "reset") {
      const confirmPassword = document.querySelector<HTMLInputElement>(".auth-modal input[name=confirm-password]")?.value || "";
      if (!password || password !== confirmPassword) { setNotice("Passwords do not match."); return; }
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === "PASSWORD_RECOVERY" && sessionUser) {
        setUserId(sessionUser.id);
  ...
  useEffect(() => {
    ...
    setReports(uniqueReports([...savedReports, ...demoReports]).filter((report) => report.id !== hiddenReportId));
  ...
  useEffect(() => {
    ...
    async function loadReports() {
      ...
      setReports(uniqueReports(data.map((report) => ({ ...report, date: report.report_date, photo: report.photo || report.name.slice(0, 2).toUpperCase() })) as Report[]).filter((report) => report.id !== hiddenReportId));
      setReportsLoading(false);
    }
    void loadReports();
  ...
  async function deleteReport(id: string) {
    ...
    if (supabase) {
      const result = await supabase.from("reports").delete({ count: "exact" }).eq("id", id).eq("owner", userId);
      if (result.error) { setNotice(`Could not delete report: ${result.error.message}`); return; }
      if (result.count !== 1) { setNotice("This report is not owned by your account or was already deleted."); return; }
    }
