...
export default function AuthPage({ mode }: { mode: AuthMode | "reset" }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = client.auth.onAuthStateChange((event, session) => {
      ...
      if (event === "SIGNED_IN" && session && authMode !== "reset") {
        void completeSession(session.user);
      }
    });
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (authMode === "register") {
      ...
      if (error) { setLoading(false); setNotice(error.message.toLowerCase().includes("already") ? "This email is already registered. Please sign in." : `Account creation failed: ${error.message}`); return; }
      if (!data.user) { setLoading(false); setNotice("Account creation did not complete. Please try again."); return; }
      if (!data.session) {
      ...
      await supabase.from("profiles").upsert({ id: data.user.id, email: data.user.email || email, registered: true, full_name: fullName || null }, { onConflict: "id" });
      await supabase.auth.signOut();
      router.push("/login");
      return;
    }
    ...
    if (error) { setLoading(false); setNotice("Invalid email or password. Create and verify an account first."); return; }
  ...
  async function verifySignup(event: FormEvent<HTMLFormElement>) {
    ...
    if (error || !data.user) { setLoading(false); setNotice(error?.message || "Email verification failed. Please try again."); return; }
    ...
    if (profileError) { setLoading(false); setNotice(`Could not finish registration: ${profileError.message}`); return; }
    await supabase.auth.signOut();
    router.push("/login");
    setVerificationCode("");
  }
    ...
    <main className="auth-page">
      <section className="auth-panel">
        ...
        {isVerify ? <form key={authMode} className="auth-form" onSubmit={verifySignup}><label>Verification code<input name="verificationCode" value={verificationCode} onChange={(event) => setVerificationCode(event.target.value.replace(/\D/g, "").slice(0, 8))} inputMode="numeric" pattern="[0-9]{6,8}" maxLength={8} placeholder="000000" autoFocus required /></label>{notice && <p className="auth-page-notice" role="alert">{notice}</p>}<button className="primary-button full" type="submit" disabled={loading}>{loading ? "Please wait..." : "Verify email"}</button><button className="text-button center" type="button" onClick={() => void resendVerificationCode()} disabled={loading || resendCooldown > 0}>{resendCooldown > 0 ? `Resend code in ${resendCooldown}s` : "Resend verification code"}</button></form> : <form key={authMode} className="auth-form" onSubmit={handleSubmit}>
          ...
          {!isForgot && <label>Password<input name="password" type="password" autoComplete={isRegister ? "new-password" : "current-password"} placeholder="At least 8 characters" required /></label>}
